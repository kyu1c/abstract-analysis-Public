...
## 🏃‍♂️ 애플리케이션 사용 가이드 (Using the Application)
서버 실행 후, 다음 절차에 따라 애플리케이션을 사용할 수 있습니다.

### 1. 회원가입 및 로그인 (Sign Up & Login)
이 프로젝트는 별도의 관리자 생성 페이지가 없으며, **모든 사용자는 동일한 회원가입 절차**를 거칩니다.

1. 웹 브라우저에서 `Login` 페이지로 이동합니다.
2. 계정이 없다면 하단의 **"Sign up"** 링크를 클릭하거나 `/signup` 경로로 이동합니다.
3. 이메일과 비밀번호를 입력하여 계정을 생성합니다.
4. 생성된 계정으로 로그인하면 메인 대시보드로 이동합니다.

### 2. 관리자 계정 활성화 (Admin Access)
위의 ["사용자 권한 및 관리자 설정"](#-사용자-권한-및-관리자-설정-user-roles--admin-setup) 섹션에서 설정한 이메일 주소로 회원가입 및 로그인을 진행하세요.

- **일반 사용자**: 로그인 후 상단 메뉴에 `My Dashboard`, `Logout` 버튼만 표시됩니다.
- **관리자**: 설정된 관리자 이메일로 로그인할 경우, 상단에 **`Admin Dashboard`** 버튼이 추가로 표시됩니다. 이 버튼을 클릭하여 관리자 전용 기능을 사용할 수 있습니다.
